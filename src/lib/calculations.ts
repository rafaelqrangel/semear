...
export function calcularIndicadores(dados: Partial<OnboardingData>): IndicadoresSemear {
  ...
  // Horas são informadas por mês (as contas da vida são mensais).
  const ttTrabalhoMes = fontes.reduce(
    (acc, f) => acc + (f.horasTrabalhoMes || 0),
    0,
  )
  const ttDeslocamentoMes = fontes.reduce(
    (acc, f) => acc + (f.horasDeslocamentoMes || 0),
    0,
  )
  const ttMes = ttTrabalhoMes + ttDeslocamentoMes // total mensal do trabalho
  const tt = ttMes / SEMANAS_POR_MES // equivalente semanal, para o tempo livre
  ...
  // total mensal (com deslocamento). É o número mais honesto.
  const vh = ttMes > 0 ? rl / ttMes : 0
  const vhBruto = ttMes > 0 ? r / ttMes : 0
  const vhTrabalho = ttTrabalhoMes > 0 ? rl / ttTrabalhoMes : 0
  const renda_passiva = dados.renda_passiva ?? 0
  const gif = c > 0 ? (renda_passiva / c) * 100 : 0

  return {
    tb: TB_SEMANAL, tab, tp, tr, tt, ttMes, ttTrabalhoMes,
